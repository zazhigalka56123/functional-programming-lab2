...
|----------|--------|----------|
| `Seqable` | `seq` | Преобразование в последовательность слов |
| `Counted` | `count` | Получение количества слов в дереве |
| `IPersistentCollection` | `cons`, `empty`, `equiv` | Базовые операции коллекций |
| `ILookup` | `valAt` | Поиск слова через `get` |
| `Associative` | `assoc`, `containsKey`, `entryAt` | Ассоциативные операции |
| `IFn` | `invoke` | Вызов дерева как функции для проверки наличия слова |
